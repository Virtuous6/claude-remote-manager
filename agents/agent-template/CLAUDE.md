...
<text>
Reply using: bash "$CRM_TEMPLATE_ROOT/core/bus/send-telegram.sh" <chat_id> "<reply>"
```
...
<text>
Reply using: bash "$CRM_TEMPLATE_ROOT/core/bus/send-message.sh" <agent> normal '<reply>' <msg_id>
```

Always include `msg_id` as reply_to (auto-ACKs the original). Un-ACK'd messages redeliver after 5 min. For no-reply messages: `bash "$CRM_TEMPLATE_ROOT/core/bus/ack-inbox.sh" <msg_id>`
...
**Soft** (preserves history): `bash "$CRM_TEMPLATE_ROOT/core/bus/self-restart.sh" --reason "why"`
**Hard** (fresh session): `bash "$CRM_TEMPLATE_ROOT/core/bus/hard-restart.sh" --reason "why"`
...
3. Create the agent:
   ```bash
   cp -r "$CRM_TEMPLATE_ROOT/agents/agent-template" "$CRM_TEMPLATE_ROOT/agents/<name>"
   cat > "$CRM_TEMPLATE_ROOT/agents/<name>/.env" << EOF
   BOT_TOKEN=<token>
   CHAT_ID=<chat_id>
   EOF
   ```
4. Enable it: `bash "$CRM_TEMPLATE_ROOT/enable-agent.sh" <name>`
...
| Action | Command |
|--------|---------|
| Enable agent | `bash "$CRM_TEMPLATE_ROOT/enable-agent.sh" <name>` |
| Disable agent | `bash "$CRM_TEMPLATE_ROOT/disable-agent.sh" <name>` |
...
| Action | Command |
|--------|---------|
| Send Telegram | `bash "$CRM_TEMPLATE_ROOT/core/bus/send-telegram.sh" <chat_id> "<msg>"` |
| Send photo | `bash "$CRM_TEMPLATE_ROOT/core/bus/send-telegram.sh" <chat_id> "<caption>" --image /path` |
| Send to agent | `bash "$CRM_TEMPLATE_ROOT/core/bus/send-message.sh" <agent> <priority> '<msg>' [reply_to]` |
| Check inbox | `bash "$CRM_TEMPLATE_ROOT/core/bus/check-inbox.sh"` |
| ACK message | `bash "$CRM_TEMPLATE_ROOT/core/bus/ack-inbox.sh" <msg_id>` |
